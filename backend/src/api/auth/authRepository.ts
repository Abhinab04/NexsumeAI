...
export class AuthRepository {
  ...
  async generateLoginToken(
    ...
    try {
      ...
      logger.error(
        `Error while generating Login Token: ${err}`,
      );
  ...
  async getLoginToken(
    ...
    try {
      ...
      const token =
        await LoginTokenModel.findOne({
          token: jwtToken,
        });

      if (!token) {
        logger.warn(
          `Login token not found: ${jwtToken}`,
        );
      ...
      if (err instanceof jwt.TokenExpiredError) {
        logger.warn(
          `Login token expired: ${jwtToken}`,
        );
      } else {
        logger.error(
          `Error while getting LoginToken: ${
  ...
  async generateRefreshTokenRefershToken(
    userId: string,
  ): Promise<{
    _id: string | null;
    email: string | null;
  ...
  async getUserFromRefreshToken(
    ...
    try {
      ...
      const user =
        await UserModel.findById(_id).exec();
      ...
      if (err instanceof jwt.TokenExpiredError) {
        logger.error(
          `Refresh Token Expired: ${incomingRefreshToken}`,
        );
      } else {
        logger.error(
